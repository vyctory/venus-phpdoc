 ...
 * @license   	https://github.com/vyctory/venus-phpdoc/blob/master/LICENSE.md Tout droit réservé à PAQUET Judicaël
 * @version   	Release: 1.0.0
 * @filesource	https://github.com/vyctory/venus-phpdoc
 * @link      	https://github.com/vyctory
 * @since     	1.0
 */
namespace VenusPhpdoc;
...
class Reader
{
     ...
     * @return array
     */
    public static function getPhpDocOfProperty($sClassName, string $propertyName) : array
    {
        $oReflectionClass  = new \ReflectionClass($sClassName);
        $reflectionProperty = $oReflectionClass->getProperty($propertyName);
        $sCommentPhpDoc = $reflectionProperty->getDocComment();

        /**
         * parse this PhpDoc @cache(param=1,param=2)
         */

        preg_match_all('/@([^\( \n\r]+)\(([^\)]+)\)/', $sCommentPhpDoc, $aMatchs, PREG_SET_ORDER);
        preg_match_all('/@([a-zA-Z\\\\]+)[\n\r]/', $sCommentPhpDoc, $aMatchs2, PREG_SET_ORDER);
        //var_dump($aMatchs,$aMatchs2);

        $aMatchs = array_merge($aMatchs, $aMatchs2);
        ...
        foreach ($aMatchs as $aOneMatch) {
            ...
            if (isset($aOneMatch[2])) {
                foreach (explode(', ', $aOneMatch[2]) as $sValue) {

                    $aFinalExplode = explode('=', $sValue);

                    if (!isset($aFinalExplode[1])) {
                        $aFinalExplode[1] = null;
                    }
                    ...
                    if ($aFinalExplode[1]{0} == "{" && $aFinalExplode[1]{strlen($aFinalExplode[1]) - 1} == "}") {
                        $aFinalExplode[1] = json_decode($aFinalExplode[1]);
                    }
